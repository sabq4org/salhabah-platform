...
## استيراد بيانات الطالبات من PDF (الآلية المعتمدة حالياً)

ملف بيانات الطالبات يأتي من نظام نور كـ PDF (مثلاً `~/Downloads/بيانات 1.pdf`).
الترميز داخل PDF غير قياسي (Arabic Presentation Forms معكوسة وحرفياً ⇄ عمودياً)،
ولذلك نستخدم استخراج بـ "خلية الجدول" (table cells) عبر PyMuPDF حتى لا تتسرب
كلمات من سطر طالبة إلى سطر طالبة أخرى.

### خطوات الاستيراد الكامل

```bash
# 1. تأكدي من تثبيت PyMuPDF محلياً
python3 -c "import fitz; print(fitz.__doc__)"

# 2. عدّلي مسار PDF داخل scripts/clean-extract-all.py إن لزم
# 3. ولّدي scripts/import-data/clean-students.json من PDF
python3 scripts/clean-extract-all.py

# 4. اعرضي بنية القاعدة قبل الاستبدال
npx tsx scripts/inspect-all.ts

# 5. استبدلي كل الطالبات + الفصول دفعة واحدة (يحترم cascade ويتأكد
#    أن attendance_records و student_grades فارغة قبل المسح)
npm run db:replace-students

# 6. تحققي من النتيجة
npm run db:verify-students
```

`clean-students.json` هو **مصدر الحقيقة الوحيد** لبيانات الطالبات؛ لا تُولِّدي
ملفات `gradeN-students.json` يدوية بعد اليوم — السكربت القديم كان يخلط أعمدة
الجدول وينتج أرقام هوية مغلوطة وأسماء مدمجة من سطور مجاورة.

### بديل تدريجي

إذا أردتِ إضافة طالبات جدد دون مسح القاعدة، يبقى `db:import-students` متاحاً
ويُمرَّر له ملف JSON بنفس تنسيق `clean-students.json`:

```bash
npm run db:import-students -- scripts/import-data/clean-students.json
```
