...
// Read every student JSON file under scripts/import-data/. The new platform
// will likely have separate files per grade level — anything matching
// `*-students.json` is considered real data.
const REAL_STUDENT_FILES_GLOB = /-students\.json$/;
const REAL_TEACHER_FILE = "scripts/import-data/teachers.json";

const KEEP_ADMIN_EMAIL = "maha@salhabah.edu.sa";
const KEEP_USERNAME = "admin";

async function run() {
  const { db, schema } = await import("../src/db/index");

  const importDir = "scripts/import-data";

  // Auto-discover every <stage>-students.json so adding a new grade file
  // doesn't require editing this script.
  const realStudentIds = new Set<string>();
  if (existsSync(importDir)) {
    for (const file of readdirSync(importDir)) {
      if (!REAL_STUDENT_FILES_GLOB.test(file)) continue;
      const rows = JSON.parse(
        readFileSync(join(importDir, file), "utf-8"),
      ) as { nationalId: string }[];
      rows.forEach((r) => realStudentIds.add(r.nationalId));
    }
  }
  const realTeacherIds = new Set<string>();
  if (existsSync(REAL_TEACHER_FILE)) {
  ...
  console.log(
    `🔎 Loaded ${realStudentIds.size} real student IDs, ${realTeacherIds.size} real teacher IDs from ${importDir}`,
  );
